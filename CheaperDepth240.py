        ...
        for mine in reversed(mines):
            ...
            if mine.y < 25:
                if myship.x - 3 < mine.x and myship.x + 29 > mine.x and self.gamestart_flag:
                    ...
                    explos.append(Explo(mine.x-3,mine.y-9))
                    explos.append(Explo(mine.x-12,mine.y-8))
                    explos.append(Explo(mine.x+4,mine.y-9))
                    explos.append(Explo(mine.x+17,mine.y-7))
                    pyxel.stop(3)
        ...
        for torpedo in reversed(torpedos):
            ...
            if round(torpedo.y) == 25:
                if myship.x - 2 < torpedo.x and myship.x + 30 > torpedo.x and self.gamestart_flag:
                    ...
                    explos.append(Explo(torpedo.x-3,torpedo.y-9))
                    explos.append(Explo(torpedo.x-12,torpedo.y-8))
                    explos.append(Explo(torpedo.x+4,torpedo.y-9))
                    explos.append(Explo(torpedo.x+17,torpedo.y-7))
                    pyxel.stop(3)
                    pyxel.play(1,[1,1,1,1,1])
                    pyxel.play(2,10)
